...
// ---- Unified relay touchpoints for domain services ----

export function onRelayStatus(
  nostr: NostrService,
  cb: (connected: number, total: number) => void,
): void {
  nostr.onRelayCountChange(cb);
}
...
export function publishReplaceable(
  ...
  return nostr.publishReplaceable(dTag, extraTags, content);
}
...
export function sendDm(
  ...
  nostr.sendDM(toPubkey, payload, expirationSecs);
}
